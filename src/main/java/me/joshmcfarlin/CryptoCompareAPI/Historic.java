...
public class Historic {
     ...
     * @throws OutOfCallsException when no more API calls are available
     */
    public static History getHour(String fromSym, String toSym, int limit, int timestamp) throws IOException, OutOfCallsException {
        String formattedUrl = String.format("https://min-api.cryptocompare.com/data/histohour?fsym=%s&tsym=%s&limit=%d&toTs=%d",
                fromSym.toUpperCase(), toSym.toUpperCase(), limit, timestamp);
        Reader r = Connection.getJSON(new URL(formattedUrl), CallTypes.HISTO);
        return new Gson().fromJson(r, History.class);
    }
